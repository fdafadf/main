...
namespace Demos.Forms.TicTacToe.Game
{
    public partial class TicTacToeGameSettingsForm : Form
    {
        ...
        public TicTacToeGameSettingsForm(IDictionary<string, Func<IActionGenerator<GameState, GameAction, Player>>> engines) : this()
        {
            noughtPlayerControl.Items.Clear();
            crossPlayerControl.Items.Clear();
            var engineList = engines.Select(e => new NamedObject<Func<IActionGenerator<GameState, GameAction, Player>>>(e.Key, e.Value)).ToArray();
        ...
        public IActionGenerator<GameState, GameAction, Player> Nought;
        public IActionGenerator<GameState, GameAction, Player> Cross;
        ...
        private IActionGenerator<GameState, GameAction, Player> LoadAIEngine(ComboBox playerControl)
        {
            if ("Human" == (playerControl.SelectedItem as string))
            {
                return null;
            }
            else if (playerControl.SelectedItem is NamedObject<Func<IActionGenerator<GameState, GameAction, Player>>> item)
            {
