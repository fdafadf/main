...
namespace Demos.Forms.Go.Game
{
    ...
    {
        public static void RefreshBoard(GoBoardControl boardControl, GameTreeNavigator<MCTreeSearchAsGameTree<GoGame, GameState, FieldCoordinates, Stone>, GameState, FieldCoordinates, Stone, MCTreeNode<FieldCoordinates, GameState>> navigator)
        {
            GameState currentState = navigator.CurrentNode.GameState;
            ...
            var mcts = navigator.GameTree.Mcts;
            var mctsWeights = mcts.GetChildrenWeights(mcts.CurrentNode);

            if (mcts.CurrentNode.Children != null)
            {
                foreach (var childNode in mcts.CurrentNode.Children)
                {
                    if (childNode.Key != FieldCoordinates.Pass)
                    {
                        if (childNode.Value.Simulations > 0)
                        {
                            boardControl.Fields[childNode.Key.X, childNode.Key.Y].AddLabel(Brushes.Magenta, $"{childNode.Value.Simulations / 10}");
                            boardControl.Fields[childNode.Key.X, childNode.Key.Y].AddLabel(Brushes.Green, $"{childNode.Value.Wins / 10}");
                        }
                    }
                }

                var mostFrequentlySimulated = mcts.CurrentNode.Children.Where(w => w.Key != FieldCoordinates.Pass && w.Value.Simulations > 0).MaxItems(i => i.Value.Simulations);
            ...
            if (node != null)
            {
                for (uint y = 0; y < playoutBoardControl.BoardSize; y++)
                {
                    for (uint x = 0; x < playoutBoardControl.BoardSize; x++)
                    {
                        ...
                        FieldState fieldState = node.GameState.InternalState.BoardFields[x, y];
                ...
                foreach (var field in node.GameState.GetAllowedActionsForRandomPlayout())
                {
